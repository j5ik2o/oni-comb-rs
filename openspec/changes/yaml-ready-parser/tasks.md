...
- [ ] 1.1 `YAML-ready` の禁止事項をテスト方針として文書化する（下流 grammar 実装で `parse_next` 直呼び、`checkpoint/reset` 直呼び、戻り値破棄を許可しない）
- [ ] 1.2 block list / indent nesting / flow-block switching / multiline block / document boundary / indent error の litmus grammar 一覧を確定する
- [ ] 1.3 litmus grammar 群を parser モジュール単体の acceptance test として追加する

## 2. State Model Redesign

- [ ] 2.1 parser が扱う state を `input state`、`checkpointable layout state`、`semantic state` に分離する設計を確定する
...
- [ ] 4.1 行頭判定、期待インデント判定、flow/block 文脈判定を public capability として設計する
- [ ] 4.2 `guard` だけでは表現できない layout-sensitive grammar 用 primitive / combinator を追加する
- [ ] 4.3 litmus grammar が新 capability だけで記述できることを確認する

## 5. Readiness Verification

- [ ] 5.1 litmus grammar 実装から命令型 escape hatch が消えていることを review で確認する
...
- [ ] 5.4 `YAML-ready` 判定後に初めて YAML クレート着手可とする方針を明文化する
