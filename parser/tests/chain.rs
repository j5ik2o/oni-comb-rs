...
fn chainl1_single_operand() {
    let mut parser = integer_parser().chainl1(
        char('+').map(|_| (|a: i64, b: i64| a + b) as fn(i64, i64) -> i64),
    );
...
fn chainr1_single_operand() {
    let mut parser = integer_parser().chainr1(
        char('^').map(|_| (|a: i64, b: i64| a.pow(b as u32)) as fn(i64, i64) -> i64),
    );
