...
fn main() {
  ...
  // 直接StaticParserを使用する例
  {
    ...
    // 単純なStaticParserの例
    let hello_parser: StaticParser<u8, String> = seq_static(b"hello").map_res(|bytes: Vec<u8>| {
    ...
    // 単純なパーサーを組み合わせる例
    let hello_world_parser: StaticParser<u8, String> = seq_static(b"hello world").map_res(|bytes: Vec<u8>| {
        std::str::from_utf8(&bytes).map(|s| s.to_string())
    });
