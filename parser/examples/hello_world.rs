...
fn main() {
  ...
  // 直接StaticParserを使用する例
  {
    ...
    let hello_parser: StaticParser<u8, String> =
      seq(b"hello").map_res(|bytes: Vec<u8>| std::str::from_utf8(&bytes).map(|s| s.to_string()));
    ...
    let hello_world_parser: StaticParser<u8, String> =
      seq(b"hello world").map_res(|bytes: Vec<u8>| std::str::from_utf8(&bytes).map(|s| s.to_string()));
