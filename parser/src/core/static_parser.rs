  ...
  F: Fn(&ParseState<'a, I>) -> ParseResult<'a, I, A> + Clone + 'a,
{
  ...
  /// 関数を適用して新しい型に変換
  pub fn map<B, G>(self, f: G) -> StaticParser<'a, I, B, impl Fn(&ParseState<'a, I>) -> ParseResult<'a, I, B> + Clone + 'a>
  where
    G: Fn(A) -> B + Clone + 'a,
    B: Clone + 'a,
  {
    ...
    StaticParser::new(move |state| {
      match method(state) {
        ParseResult::Success { value, length } => {
          ParseResult::successful(f_clone(value), length)
        }
  ...
  /// 条件に基づいてフィルタリング
  pub fn with_filter<G>(self, f: G) -> StaticParser<'a, I, A, impl Fn(&ParseState<'a, I>) -> ParseResult<'a, I, A> + Clone + 'a>
  where
    G: Fn(&A) -> bool + Clone + 'a,
    I: Clone + 'a,
  {
  ...
  /// 条件の否定に基づいてフィルタリング
  pub fn with_filter_not<G>(self, f: G) -> StaticParser<'a, I, A, impl Fn(&ParseState<'a, I>) -> ParseResult<'a, I, A> + Clone + 'a>
  where
    G: Fn(&A) -> bool + Clone + 'a,
    I: Clone + 'a,
  {
    ...
    B: 'a,
  {
    ...
    StaticParser::new(move |state| {
      ...
      match method(state) {
        ...
        ParseResult::Failure { error: _, committed_status: _ } => {
...
impl<'a, I, A> Parser<'a, I, A> {
  pub fn to_static_parser(self) -> StaticParser<'a, I, A, impl Fn(&ParseState<'a, I>) -> ParseResult<'a, I, A> + Clone + 'a> {
