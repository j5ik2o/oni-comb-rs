...
pub fn take<'a>(n: usize) -> Take<StrInput<'a>> {
  crate::primitive::take::take(n)
}

pub fn take_while0<'a, F: FnMut(char) -> bool>(f: F) -> TakeWhile0<F, StrInput<'a>> {
  crate::primitive::take_while0::take_while0(f)
}

pub fn take_while1<'a, F: FnMut(char) -> bool>(f: F) -> TakeWhile1<F, StrInput<'a>> {
