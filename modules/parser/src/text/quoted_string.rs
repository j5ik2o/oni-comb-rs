...
impl<'a> Parser<StrInputStream<'a>> for QuotedString {
  ...
  fn parse_next(&mut self, input: &mut StrInputStream<'a>) -> PResult<Cow<'a, str>, ParseError> {
    let origin = input.position();
    let remaining = input.as_str();
    let bytes = remaining.as_bytes();

    if bytes.is_empty() || bytes[0] != b'"' {
      return Err(Fail::Backtrack(ParseError::from_position(origin, Expected::Char('"'))));
    }
      ...
      match chars.next() {
        ...
        Some('\\') => {
          ...
          match chars.next() {
            ...
            Some('u') => {
              consumed += 1;
              let code = parse_hex4(&mut chars, &mut consumed, input)?;
              ...
              if (0xD800..=0xDBFF).contains(&code) {
                ...
                match (c1, c2) {
                  (Some('\\'), Some('u')) => {
                    let low = parse_hex4(&mut chars, &mut consumed, input)?;
                    if !(0xDC00..=0xDFFF).contains(&low) {
                      return Err(Fail::Cut(ParseError::from_position(
                        input.position_after(consumed - 4),
                    ...
                    match char::from_u32(cp) {
                      ...
                      None => {
                        return Err(Fail::Cut(ParseError::from_position(
                          input.position_after(consumed - 10),
