...
fn byte_recursive_nested_parens() {
  let parser = recursive::<ByteInputStream, &[u8], ParseError, _, _>(|rec| {
    let inner = take_while1::<ByteInputStream, _>(|b: u8| b.is_ascii_alphabetic());
    // ( rec ) or inner
    let parens = satisfy::<ByteInputStream, _>(|b: u8| b == b'(')
      .zip_right(rec)
      .zip_left(satisfy::<ByteInputStream, _>(|b: u8| b == b')'));
    parens.or(inner)
  });
...
fn byte_recursive_fail() {
  ...
  let mut p = parser;
  let mut input = ByteInputStream::new(b"123");
