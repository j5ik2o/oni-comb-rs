...
fn byte_input_take_3() {
  ...
  let mut input = ByteInputStream::new(b"abcdef");
...
fn byte_input_take_not_enough() {
  let mut p = take::<ByteInputStream>(5);
...
fn byte_input_satisfy_match() {
  let mut p = satisfy::<ByteInputStream, _>(|b: u8| b.is_ascii_uppercase());
...
fn byte_input_satisfy_empty() {
  let mut p = satisfy::<ByteInputStream, _>(|_: u8| true);
...
fn byte_input_take_while0_digits() {
  let mut p = take_while0::<ByteInputStream, _>(|b: u8| b.is_ascii_digit());
...
fn byte_input_take_while1_digits() {
  let mut p = take_while1::<ByteInputStream, _>(|b: u8| b.is_ascii_digit());
  let mut input = ByteInputStream::new(b"42abc");
...
fn byte_input_take_while_n_m_bounded() {
  let mut p = take_while_n_m::<ByteInputStream, _>(2, 4, |b: u8| b.is_ascii_digit());
  let mut input = ByteInputStream::new(b"12345");
...
fn byte_input_take_while_n_m_below_min() {
  let mut p = take_while_n_m::<ByteInputStream, _>(3, 5, |b: u8| b.is_ascii_digit());
  let mut input = ByteInputStream::new(b"12abc");
  assert!(p.parse_next(&mut input).is_err());
