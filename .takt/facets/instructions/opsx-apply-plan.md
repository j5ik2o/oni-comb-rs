タスクファイルを読み込み、次のタスクバッチの実行計画を立てよ。

**注意:** Previous Responseがある場合は前回バッチの完了後の再計画である。
タスクのチェック状況を確認し、次のバッチを決定せよ。

**やること:**

1. 変更名を特定する
   - 初回（00-plan.md がない）: User Task から変更名を抽出する
   - 再計画（00-plan.md がある）: 既存 00-plan.md の「## 対象変更」を優先する
...
3. CLIの `tasksFile` フィールドから `tasks_path` を決定する

4. `{tasks_path}` を読み込む
   - ファイルが存在しない場合は「tasks.mdが存在しない、またはブロック状態」と判定する
...
7. 未完了タスクがある場合:
   ...
   d. `contextFiles` に記載されたコンテキストファイルを読み、実装のガイダンスを得る
